    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            try:
                ...
                if mermaid_code:
                    # 显示结果
                    # 为每个文件生成唯一的 session state key
                    mermaid_key = f"mermaid_code_{uploaded_file.name}"
                    if mermaid_key not in st.session_state:
                        st.session_state[mermaid_key] = mermaid_code
                    ...
                    with st.expander(f"结果: {uploaded_file.name}", expanded=True):
                        ...
                        st.markdown("##### Mermaid 流程图 (可编辑)")
                        # 使用 text_area 实现编辑功能，并绑定到 session_state
                        edited_mermaid_code = st.text_area(
                            "编辑 Mermaid 代码:",
                            value=st.session_state[mermaid_key],
                            key=f"edit_{mermaid_key}", # 唯一 key
                            height=200,
                            label_visibility="collapsed" # 隐藏标签，因为上面有标题了
                        )
                        # 更新 session_state
                        ...
                        st.markdown(
                            f'<a href="{mermaid_editor_url}" target="_blank">在 Mermaid Live 编辑器中打开并下载</a>',
